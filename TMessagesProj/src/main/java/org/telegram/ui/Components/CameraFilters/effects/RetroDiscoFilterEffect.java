...
public class RetroDiscoFilterEffect implements ICameraFilterEffect {
    @Override
    public int getFilterType() {
        return CameraFilterType.RETRO_DISCO;
    }
    ...
    public void process(int[] in, int[] out, int w, int h, float intensity) {
        for (int i = 0; i < in.length; i++) {
            ...
            float outR = r * 1.25f + 15f;
            float outG = g * 0.90f + 5f;
            float outB = b * 1.30f + 25f;
