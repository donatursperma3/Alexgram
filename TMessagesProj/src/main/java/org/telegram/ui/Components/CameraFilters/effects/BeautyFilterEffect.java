...
public class BeautyFilterEffect implements ICameraFilterEffect {
    @Override
    public int getFilterType() {
        return CameraFilterType.BEAUTY;
    }
    ...
    public void process(int[] in, int[] out, int w, int h, float intensity) {
        for (int i = 0; i < in.length; i++) {
            ...
            float outR = r * 1.08f + 10.2f;
            float outG = g * 1.04f + 5.1f;
            float outB = b * 1.00f + 2.5f;
