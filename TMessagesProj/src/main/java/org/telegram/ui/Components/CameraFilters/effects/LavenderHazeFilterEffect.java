...
public class LavenderHazeFilterEffect implements ICameraFilterEffect {
    @Override
    public int getFilterType() {
        return CameraFilterType.LAVENDER_HAZE;
    }
    ...
    public void process(int[] in, int[] out, int w, int h, float intensity) {
        for (int i = 0; i < in.length; i++) {
            ...
            float outR = r * 1.10f + 16f;
            float outG = g * 0.92f + 4f;
            float outB = b * 1.22f + 24f;
