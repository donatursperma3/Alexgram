...
public class Polaroid600FilterEffect implements ICameraFilterEffect {
    @Override
    public int getFilterType() {
        return CameraFilterType.POLAROID_600;
    }
    ...
    public void process(int[] in, int[] out, int w, int h, float intensity) {
        for (int i = 0; i < in.length; i++) {
            ...
            float outR = r * 0.95f + 22f;
            float outG = g * 1.02f + 16f;
            float outB = b * 0.92f + 20f;
