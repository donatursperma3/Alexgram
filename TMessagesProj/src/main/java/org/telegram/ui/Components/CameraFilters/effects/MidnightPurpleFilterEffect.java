...
public class MidnightPurpleFilterEffect implements ICameraFilterEffect {
    @Override
    public int getFilterType() {
        return CameraFilterType.MIDNIGHT_PURPLE;
    }
    ...
    public void process(int[] in, int[] out, int w, int h, float intensity) {
        for (int i = 0; i < in.length; i++) {
            ...
            float outR = r * 0.90f + 25f;
            float outG = g * 0.45f + 4f;
            float outB = b * 1.35f + 45f;
