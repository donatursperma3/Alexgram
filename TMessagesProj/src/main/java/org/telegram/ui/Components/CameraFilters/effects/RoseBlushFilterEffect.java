...
public class RoseBlushFilterEffect implements ICameraFilterEffect {
    @Override
    public int getFilterType() {
        return CameraFilterType.ROSE_BLUSH;
    }
    ...
    public void process(int[] in, int[] out, int w, int h, float intensity) {
        for (int i = 0; i < in.length; i++) {
            ...
            float outR = r * 1.20f + 20.4f;
            float outG = g * 0.95f + 7.6f;
            float outB = b * 1.10f + 15.3f;
