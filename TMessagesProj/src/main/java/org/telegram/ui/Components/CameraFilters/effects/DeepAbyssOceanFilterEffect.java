...
public class DeepAbyssOceanFilterEffect implements ICameraFilterEffect {
    @Override
    public int getFilterType() {
        return CameraFilterType.DEEP_ABYSS_OCEAN;
    }
    ...
    public void process(int[] in, int[] out, int w, int h, float intensity) {
        for (int i = 0; i < in.length; i++) {
            ...
            float outR = r * 0.40f;
            float outG = g * 1.15f + 10f;
            float outB = b * 1.35f + 35f;
