...
public class KodakGoldFilterEffect implements ICameraFilterEffect {
    @Override
    public int getFilterType() {
        return CameraFilterType.KODAK_GOLD;
    }
    ...
    public void process(int[] in, int[] out, int w, int h, float intensity) {
        for (int i = 0; i < in.length; i++) {
            ...
            float outR = r * 1.15f + 14f;
            float outG = g * 1.06f + 8f;
            float outB = b * 0.88f + 2f;
