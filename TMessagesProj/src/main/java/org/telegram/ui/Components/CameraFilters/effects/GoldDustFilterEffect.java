...
public class GoldDustFilterEffect implements ICameraFilterEffect {
    @Override
    public int getFilterType() {
        return CameraFilterType.GOLD_DUST;
    }
    ...
    public void process(int[] in, int[] out, int w, int h, float intensity) {
        for (int i = 0; i < in.length; i++) {
            ...
            float outR = r * 1.22f + 20f;
            float outG = g * 1.10f + 14f;
            float outB = b * 0.70f + 2f;
